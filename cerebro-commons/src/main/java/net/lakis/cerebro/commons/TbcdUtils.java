...
public class TbcdUtils {
	...
	private TbcdUtils(byte[] tbcd, int start, int length) {
		this.tbcd = tbcd;
		this.start = start;
		this.length = length;
	}

	private TbcdUtils(String number) {
		this.number = number;
	}

	public static TbcdUtils from(byte[] tbcd, int start, int length) {
		return new TbcdUtils(tbcd, start, length);
	}

	public static TbcdUtils from(byte[] tbcd, int start) {
		return new TbcdUtils(tbcd, start, tbcd.length - start);

	}

	public static TbcdUtils from(byte[] tbcd) {
		return new TbcdUtils(tbcd, 0, tbcd.length);

	}

	public static TbcdUtils from(String tbcd) {
		return new TbcdUtils(tbcd);

	}

	public TbcdUtils odd(boolean odd) {
		this.odd = odd;
		return this;
	}

	public TbcdUtils filler(byte filler) {
		this.filler = filler;
		return this;
	}
